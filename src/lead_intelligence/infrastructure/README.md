...
| `search/` | Adapters that implement `application/ports/search_provider_port.py` — the Search Layer's provider family (sibling to, and deliberately separate from, `enrichment/`). See `application/search/README.md`'s "Federated Search" section for why. |
| `search/company_crawler/` | **Implemented.** `CompanyCrawlerProvider` — one of five Federated Search providers: crawls the executive's own company website (leadership/management/board/about/team/people/press/news pages) and returns `SearchResult`s only, confidence `1.00`. See `search/company_crawler/README.md`. |
| `search/press_release/` | **Implemented.** `PressReleaseProvider` — crawls the same company website scoped to press/newsroom/media/investor-relations/announcements pages, confidence `0.95`. See `search/press_release/README.md`. |
| `search/linkedin/` | **Implemented.** `LinkedInSearchProvider` — searches Google's index of LinkedIn's public profile pages (restricted to `linkedin.com`) for profile changes, confidence `0.98`. See `search/linkedin/README.md`. |
| `search/news/` | **Implemented.** `NewsProvider` — searches trusted business-news domains (Reuters/Bloomberg/Yahoo Finance/BusinessWire/PRNewswire/GlobeNewswire) for executive-movement news, confidence `0.94`/`0.92`. See `search/news/README.md`. |
| `search/google/` | **Implemented.** `GoogleSearchProvider` (Search Layer) — general web search via the Google Custom Search API for inflection-signal keywords, confidence `0.80`. See `search/google/README.md`. |
| `search/google_custom_search/` | **Implemented.** `GoogleCustomSearchClient` — the shared, retry/cache-guarded Google Custom Search JSON API client reused by `search/google/`, `search/linkedin/`, and `search/news/`. |
| `search/browser/` | **Implemented, but no longer wired into `run_pipeline.py`.** `BrowserSearchProvider` — drives a real, headless browser (Playwright) against a configured search-results page and returns `SearchResult`s only. Predates the Federated Search redesign; kept for standalone use. See `search/browser/README.md`. |
| `search/extraction/` | **Implemented.** `SearchExtractionEngine` — the RFC's Page Fetcher + Content Extractor + Observation Extraction stages: turns `SearchResult` URLs into `ObservationCandidate`s via deterministic HTML parsing and named rule-based fact patterns, now also detecting `event_keywords` and carrying `source_provider`/`confidence`/`raw_text`/`evidence_type` (no AI, no PDFs). See `search/extraction/README.md`. |
