...
| `executive_pipeline/` | The Executive Processing Pipeline (Version 2) — wires Identity Resolution, the Enrichment Provider Framework, the Search Layer + Search Extraction Engine, the Executive Comparison Engine, the Inflection Detection Engine, and (if configured) the Contact Verification Framework together to process one executive — or, via `process_batch()`, a whole batch — end to end into one Final Executive Intelligence Report. No new provider, no framework redesign — pure orchestration. See `executive_pipeline/README.md`. |
...
| `search/` | The Search Layer (Version 1, per the approved Search Layer RFC) — provider registry, priority ordering, health tracking (reused from `enrichment/`), and coordination across multiple search sources, structurally separate from the Enrichment Provider Framework so a search provider can never return an observation. Its first concrete provider, Browser Search, is implemented in `infrastructure/search/browser/`; its Search Extraction Engine (`infrastructure/search/extraction/`) turns found URLs into evidence. Both are wired into `executive_pipeline/` as of Version 2. See `search/README.md`. |
...
Executive Processing Pipeline's, Evaluation & Validation module's, and
Search Layer's use cases, ports, and DTOs are implemented — see
...
`executive_pipeline/README.md`, `evaluation/README.md`, and
`search/README.md` for the full picture. `services/` remains empty on
purpose; no other use cases exist yet.
