...
| `enrichment/` | The Enrichment Provider Framework (Version 1) — provider registry, priority ordering, refresh policy, health tracking, and coordination across multiple future external sources. Framework only; no concrete provider (no web scraping, no external API calls) exists yet. See `enrichment/README.md`. |

## Current status
The Import Engine's, Cleaning Engine's, Identity Resolution Engine's, and
Enrichment Provider Framework's use cases, ports, and DTOs are implemented
— see `infrastructure/importers/README.md`, `cleaning/README.md`,
`identity_resolution/README.md`, and `enrichment/README.md` for the full
picture. `services/` remains empty on purpose; no other use cases exist yet.
